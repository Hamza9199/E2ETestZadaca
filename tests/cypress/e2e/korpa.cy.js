describe('Korpa (Cart) E2E', () => {
  ...
  it('should add a product to cart and display it in Korpa', () => {
    cy.visit(`http://localhost:5173/proizvod/${productId}`);
    ...
    cy.visit('http://localhost:5173/korpa');
    cy.reload();
    ...
    cy.get('[data-testid="korpa-item"]').first().find('[data-testid="remove-btn"]').click();

    // Provjeri da je korpa prazna
    cy.get('[data-testid="korpa-empty"]').should('contain.text', 'Korpa je prazna');
  });
